...
public class RepositotyViewFactory {
    ...
    public static RepositoryView[] get(int count){
        ...
        for(int i =0 ;i<count;i++){
            res[i] = RepositoryViewBuilder.newRepositoryView().id(i).repositoryName(FULL_REPONAME+i).repositoryUrl(REPOURL+i).stars(rnd.nextInt(max)+min).description("Description"+i).build();
        }
        return  res;
    }
